...
function NutritionLayout({ name, image, description, actionButton, actionLink }) {

  return (
    <div className="exercise-layout bg-white/95 rounded-lg p-6 border-4 border-black mb-8">
            ...
            <div className="exercise-description flex-1 md:pl-4 md:border-l-2 md:border-black flex flex-col"> 
                ...
                {actionButton && actionLink && (
                  <Link to={actionLink} className="mt-auto">
                    <button className="bg-black hover:bg-gray-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors duration-300">
                      {actionButton}
                    </button>
                  </Link>
                )}
